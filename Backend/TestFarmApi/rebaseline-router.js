...
// Run git without blocking the event loop, so streamed progress can flush between steps.
...
router.post('/rebaseline', async (req, res) => {
  ...
  // Write one event and wait until it is flushed to the socket before continuing, so each step is
  // delivered live rather than batched at the end.
  const send = (event) => new Promise((resolve) => {
    const ok = res.write(JSON.stringify(event) + '\n', () => setImmediate(resolve));
    if (!ok) {
      // Backpressure: wait for the buffer to drain instead of piling up.
      res.once('drain', () => setImmediate(resolve));
    }
  });
  ...
  try {
    if (!Array.isArray(TestResultIds) || TestResultIds.length === 0) {
      await send({ type: 'error', message: 'No tests were selected for rebaseline.' });
      return;
    }
    ...
    if (results.length === 0) {
      await send({ type: 'error', message: 'Selected test results were not found.' });
      return;
    }
    ...
    if (!repositoryName) {
      await send({ type: 'error', message: 'Could not determine the repository for the selected tests.' });
      return;
    }

    const repository = await Repository.findOne({ where: { Name: repositoryName } });
    if (!repository) {
      await send({ type: 'error', message: `Repository "${repositoryName}" does not exist.` });
      return;
    }

    await send({ type: 'run-start', repositoryName, total: results.length });
    ...
    await send({ type: 'clone', message: `Cloning ${repositoryName}...` });
    fs.mkdirSync(appSettings.storage.repositories, { recursive: true });
    await execFileAsync('git', ['clone', connectionString, cloneDir]);
    // Author identity used for the rebaseline commit.
    await git(cloneDir, ['config', 'user.name', authorName]);
    await git(cloneDir, ['config', 'user.email', authorEmail]);
    ...
    for (const result of results) {
      ...
      await send({ type: 'test-start', testResultId: result.Id, testName, testPath });

      // Passing tests need no rebaseline.
      if ((result.Status || '').toLowerCase() === 'passed') {
        await send({ type: 'test-passed', testResultId: result.Id, testName });
        continue;
      }

      if (!testPath) {
        await send({ type: 'test-nothing-to-rebaseline', testResultId: result.Id, testName, reason: 'Test has no repository path.' });
        continue;
      }
      ...
      if (!fs.existsSync(testConfigPath)) {
        await send({ type: 'test-nothing-to-rebaseline', testResultId: result.Id, testName, reason: 'test.testfarm not found in repository.' });
        continue;
      }
      ...
      try {
        ...
        await send({ type: 'test-nothing-to-rebaseline', testResultId: result.Id, testName, reason: `Could not parse test.testfarm: ${e.message}` });
        continue;
      }

      if (diffPairs.length === 0) {
        await send({ type: 'test-nothing-to-rebaseline', testResultId: result.Id, testName, reason: 'Test defines no diffs to rebaseline (likely an execution error).' });
        continue;
      }
      ...
      if (rebaselinablePairs.length === 0) {
        ...
        await send({ type: 'test-nothing-to-rebaseline', testResultId: result.Id, testName, reason });
        continue;
      }
      ...
      if (!fs.existsSync(archivePath)) {
        await send({ type: 'test-nothing-to-rebaseline', testResultId: result.Id, testName, reason: 'Result archive is missing - nothing to extract.' });
        continue;
      }
      ...
      try {
        ...
        await send({ type: 'test-nothing-to-rebaseline', testResultId: result.Id, testName, reason: `Failed to extract archive: ${e.message}` });
        continue;
      }
      ...
      for (const pair of rebaselinablePairs) {
        ...
        await git(cloneDir, ['add', '--', gitPath]);

        replaced.push({ diff: diffName, goldPath: gitPath });
        await send({ type: 'diff-replaced', testResultId: result.Id, testName, diff: diffName, goldPath: gitPath });
      }

      if (replaced.length > 0) {
        totalFilesStaged += replaced.length;
        await send({ type: 'test-rebaselined', testResultId: result.Id, testName, replacedCount: replaced.length, replaced, skipped });
      } else {
        await send({
    ...
    if (totalFilesStaged === 0) {
      await send({ type: 'complete', committed: false, filesChanged: 0, message: 'Nothing to rebaseline - no baseline files were changed.' });
      return;
    }

    const commitMessage = `Rebaseline ${totalFilesStaged} gold file(s) from tests run #${testRunId} by ${authorName}`;
    await send({ type: 'commit', message: `Committing ${totalFilesStaged} file(s)...` });
    await git(cloneDir, ['commit', '-m', commitMessage, '--author', `${authorName} <${authorEmail}>`]);

    await send({ type: 'push', message: 'Pushing to repository...' });
    await git(cloneDir, ['push']);

    await send({ type: 'complete', committed: true, filesChanged: totalFilesStaged, message: `Pushed ${totalFilesStaged} rebaselined file(s) to ${repositoryName}.` });
  } catch (error) {
    console.error('Error during rebaseline:', error);
    const detail = (error.stderr && error.stderr.toString()) || error.message || String(error);
    await send({ type: 'error', message: `Rebaseline failed: ${detail}` });
  } finally {
    rmDir(cloneDir);
